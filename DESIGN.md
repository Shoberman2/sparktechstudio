...
- **The one exception:** the hero is a dark surface (`#0a0907`) — the only one on the site. Its visual is a **live terminal session**, not an image or a video: the pitch sits left, a `build.sh` panel prints its output line by line on the right. No photography anywhere on the site.
...
## Motion
- **Hero session:** the `build.sh` panel prints one line every 260ms after a 700ms beat, then parks on a blinking caret. Lines appear whole, not char-by-char — real command output prints by the line, and typing it out character-wise reads as fake. Reduced motion renders all lines immediately.
